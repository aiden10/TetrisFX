...
public class TetrisApp extends Application {
    ...
    public void start(Stage stage) {
        ...
        Timeline fall = new Timeline(
                new KeyFrame(Duration.seconds(0.4), event -> { // do every 0.75 seconds
        ...
        Timeline check = new Timeline(
                new KeyFrame(Duration.seconds(0.0005), event -> { // doing so constantly seems like a bad idea
                    currentPieceType = currentPiece.get().getType();
                    draw();
                    shouldClearLine();
                    if (shouldClear) {
                    ...
                    if (currentPiece.get().isStop()) {
                        ...
                        if (random != 1) {
                            random = (int) (Math.random() * 7); // make it more likely to get I pieces
                        }
    ...
    public void clearLine() {
        ...
        occupiedSquares.removeAll(toRemove); // remove the squares on the row which should be cleared
        for (List<Integer> occupiedSquare : occupiedSquares) {
            if (occupiedSquare.get(1) < clearedY) { // check if any of the occupied squares are above the cleared y line
                System.out.println("occupiedSquare.get(1) " + occupiedSquare.get(1));
                System.out.println("clearedY " + clearedY);
                occupiedSquare.set(1, occupiedSquare.get(1) + 35); // if they are then move each one down 35
                score += 100;
            }
            shouldClear = false;
        }
